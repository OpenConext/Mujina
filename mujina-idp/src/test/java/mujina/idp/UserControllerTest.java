...
public class UserControllerTest extends AbstractIntegrationTest {
  ...
  private String slsEndpoint = "http://localhost:9090/saml/SLS";
  ...
  private void doSLO(boolean post, CookieFilter cookieFilter, boolean ok) {
    ...
    if (post) {
      needStatus = ok ? SC_OK : SC_MOVED_TEMPORARILY;
    } else {
      needStatus = SC_OK;
    }
    ...
    if (ok) {
      ...
      if (post) {
        ...
        XmlPath xmlPath = new XmlPath(XmlPath.CompatibilityMode.HTML, stringResponse);
        String samlResponse = xmlPath.get("html.body.form.div.input.findAll{it.@name == 'SAMLResponse'}[0].@value").toString();
        xmlPath = new XmlPath(new String(Base64.decode(samlResponse)));
      ...
      if (!post) {
        ...
        // make sure we get a login form, not a logout form.
        String stringResponse = response.getBody().asString();
        XmlPath xmlPath = new XmlPath(XmlPath.CompatibilityMode.HTML, stringResponse);
        Assert.assertEquals("Login page", xmlPath.get("html.head.title.text()").toString());
