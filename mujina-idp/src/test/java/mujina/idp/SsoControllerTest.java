...
public class SsoControllerTest extends AbstractIntegrationTest {
  ...
  @Before
  public void setup() {
    idpConfiguration.putAttribute(USERNAME_ATTRIBUTE, "j.doe@example.com");
  }
  ...
  private void doSingleSignOn(boolean post) throws Exception {
    ...
    assertThat(String.format("SAML response should contain the %s attribute", "j.doe@example.com"), samlResponse.contains("j.doe@example.com"));
    assertThat(String.format("SAML response should contain the %s attribute value", USERNAME_ATTRIBUTE), samlResponse.contains(USERNAME_ATTRIBUTE));
    assertThat(String.format("SAML response should not contain the %s attribute", TEST_ATTRIBUTE), !samlResponse.contains(TEST_ATTRIBUTE));
  }
}
